...
GrimmLevel::GrimmLevel()
{
	...
	GameplayStatics::CreateObject<Bg>({ 1350,800 })->Init("grimmtent_extrance", -1, -1);
	GameplayStatics::CreateObject<Bg>({ 1400,820 })->Init("grimmtent_extrance_front", 5);
	GameplayStatics::CreateObject<Bg>({ 1330,875 })->Init("grimmtent_extrance_light", 2);

	for (int i = 0; i < 4; i++)
	{
		GameplayStatics::CreateObject<Bg>({ 750.f * (1.5f - i) - 100.f,1000.f })->Init("grimmtent_ground_0", 2, 2);
	}

	for (int i = 0; i < 5; i++)
	{
		GameplayStatics::CreateObject<Bg>({ 680.f * (2.f - i) - 100.f,1010.f })->Init("grimmtent_ground_1", 3, 3);
	}

	GameplayStatics::CreateObject<Bg>({ -1000.f,990.f })->Init("grimmtent_ground_2", 5, 5);
	GameplayStatics::CreateObject<Bg>({ 1500.f,975.f })->Init("grimmtent_ground_3", 5, 5);
	GameplayStatics::CreateObject<Bg>({ 950.f,1010.f })->Init("grimmtent_ground_2", 4, 4);
	GameplayStatics::CreateObject<Bg>({ -300.f,1025.f })->Init("grimmtent_ground_3", 5, 5);
	GameplayStatics::CreateObject<Bg>({ 230.f,950.f })->Init("grimmtent_short_pole", 4, 4);
	GameplayStatics::CreateObject<Bg>({ -335.f,410.f })->Init("grimmtent_curtain_", 4);

	for (int i = 0; i < 3; i++)
	{
		GameplayStatics::CreateObject<Bg>({ 550.f * (2.f - i),930.f })->Init("grimmtent_wallpaper", -3, -3);
		GameplayStatics::CreateObject<Bg>({ 550.f * (2.f - i),675.f })->Init("grimmtent_ceiling", -2, -2);
		GameplayStatics::CreateObject<Bg>({ 550.f * (2.f - i),615.f })->Init("grimmtent_ceiling_0", 3);
		GameplayStatics::CreateObject<Bg>({ 550.f * (1.5f - i),725.f })->Init("grimmtent_ceiling_1", 4);
		GameplayStatics::CreateObject<Bg>({ 550.f * (2.f - i) - 275.f,820.f })->Init("grimmtent_curtain", -1, -1);
		GameplayStatics::CreateObject<Bg>({ 500.f * (2.f - i),825.f })->Init("grimmtent_pole_standard", 5, 5);
